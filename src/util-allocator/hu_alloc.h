...
#if defined(RESERVE_ADDRESS_SPACE_128)
const yint RESERVE_ADDRESS_SPACE = 128;
#elif defined(RESERVE_ADDRESS_SPACE_256)
const yint RESERVE_ADDRESS_SPACE = 256;
#else
const yint RESERVE_ADDRESS_SPACE = 640; // 640 gb ought to be enough for anybody
#endif
...
const yint LARGE_GROUP_COUNT = (1ull << (30 - LARGE_GROUP_SIZE_LN)) * RESERVE_ADDRESS_SPACE;
...
typedef ui16 TLargeAllocChunkCount;
static TLargeAllocChunkCount *LargeAllocChunkCount;
...
static void LargeInit()
{
    ...
    LargeAllocChunkCount = (ui16*)OsAlloc(sizeof(TLargeAllocChunkCount) * LARGE_CHUNK_COUNT);
...
static ui64 MakeAllocMask(yint len)
{
    if (len == 64) {
        return -1ll;
    }
...
static bool LargeTryAllocChunks(yint largeBlockId, yint chunkCount, yint minAdditionalFreeChunks, char **res)
{
    ...
    if (bmask.FreeChunkMask.compare_exchange_weak(expected, newMask)) {
        ...
        LargeAllocChunkCount[largeChunkId] = (TLargeAllocChunkCount)chunkCount;
...
const yint SEGMENT_COUNT = (1ull << (30 - SEGMENT_SIZE_LN)) * RESERVE_ADDRESS_SPACE;
...
struct TSegmentSizeContext
{
    ...
    TChunkOffset ChunkOffset; // pointer to the current chunk within segment
    TChunkOffset ChunkOffsetMask;
...
typedef ui8 TAllocLevel;
const TAllocLevel AL_FREE = 0;
const TAllocLevel AL_SYSTEM = 0x7f;
const TAllocLevel AL_MEDIUM_ALLOC = 0x77;

struct TSegmentHeader
{
    TAllocLevel CurAllocLevel[BLOCK_COUNT]; // secondary structure, just to determine allocation size, primary info if block is free is stored at GlobalFreeBlockMask
...
static bool BlockAlloc(yint pow2, ui64 allowMask, yint *res)
{
    ...
    for (;;) {
        ...
        if (_BitScanForward64(&freePtr, segHdr->LocalFreeBlockMask & allowMask)) {
            ...
            segHdr->CurAllocLevel[freePtr] = (TAllocLevel)pow2;
    ...
    for (;; SwitchSegment(ALL_BLOCKS, 1)) {
        ...
        if (segment) {
            ...
            if (!_BitScanForward64(&idx, sCtx.LocalFreeAllocMask)) {
                ...
                sCtx.ChunkOffset = TSegmentSizeContext::TChunkOffset(block << BLOCK_SIZE_LN);
            }
    ...
    for (;; SwitchSegment(ALL_2LAYER_BLOCKS, 1)) {
        ...
        if (segment) {
            ...
            if (!_BitScanForward64(&idx, sCtx.LocalFreeAllocMask)) {
                ...
                yint block = GetBlockId(sCtx.ChunkOffset);
                yint chunk = GetChunkId(sCtx.ChunkOffset, (ui8)pow2);
                ...
                sCtx.ChunkOffset = TSegmentSizeContext::TChunkOffset(block * BLOCK_SIZE + (chunk << (6 + pow2)));
        ...
        if (pow2 < (ui8)NEED_2LAYER) {
            ...
            if (segment == pThreadSegment) {
                ...
                if ((memPtr & sCtx.ChunkOffsetMask) == sCtx.ChunkOffset) {
                    ...
                    ui8 idx = (ui8)((memPtr & ~sCtx.ChunkOffsetMask) >> pow2);
                    Y_ASSERT(idx < 64);
