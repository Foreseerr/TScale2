...
namespace NNet
{
#define CHECK_Z(x) { int rv = (x); if (rv != 0) { DebugPrintf("check_z failed, errno = %d\n", errno); Y_VERIFY(0); } }


//////////////////////////////////////////////////////////////////////////
const int MAX_SGE = 1;
const size_t MAX_INLINE_DATA_SIZE = 16;
const int MAX_OUTSTANDING_RDMA = 16;
...
public:
    TIBContext(ibv_device *device) : Lock(0)
    {
    ...
    bool IsValid() const { return Context != 0 && ProtDomain != 0; }

    class TLock
    {
        TIntrusivePtr<TIBContext> Ptr;
        TGuard<TAtomic> Guard;
    public:
        TLock(TPtrArg<TIBContext> ctx) : Ptr(ctx), Guard(ctx->Lock) {}
        TPtrArg<TIBContext> GetOwner() { return Ptr; }
...
public:
    TIBPort(TIBContext::TLock &ibContext, int port, int busId) : IBCtx(ibContext.GetOwner()), BusId(busId)
    {
    ...
    TPtrArg<TIBContext> GetCtx() { return IBCtx; }
...
public:
 ...
 /*      ibv_cq_init_attr_ex attr;
        ...
        if (vcq) {
            ...
            CQ = ibv_create_cq(ibContext.GetContext(), maxCQEcount, 0, 0, 0);
 //       }
    }
    ibv_cq *GetCQ()
    {
...
public:
    TMemoryRegion(TPtrArg<TIBContext> ctx, void *buf, size_t len)
        : IBCtx(ctx)
    {
        TIBContext::TLock ibContext(IBCtx);
        MR = ibv_reg_mr(
            ibContext.GetProtDomain(),
            buf, len,
            IBV_ACCESS_LOCAL_WRITE|IBV_ACCESS_REMOTE_WRITE|IBV_ACCESS_REMOTE_READ|IBV_ACCESS_RELAXED_ORDERING//|IBV_ACCESS_ALLOCATE_MR // somehow undefined?
            );
        Y_ASSERT(MR);
    }
    TMemoryRegion(TPtrArg<TIBContext> ctx, ui64 bufAddr, int dmaBuf, size_t len) : IBCtx(ctx)
    {
        TIBContext::TLock ibContext(IBCtx);
        ui64 bufOffset = 0;
        ui64 iova = bufAddr;
        MR = ibv_reg_dmabuf_mr(ibContext.GetProtDomain(), bufOffset, len, iova, dmaBuf,
    ...
    char *GetData() { return MR ? (char*)MR->addr : 0; }
    bool IsCovered(const void *data, size_t len) const
    {
        size_t dataAddr = (const char*)data - (const char*)0;
        size_t bufAddr = (const char*)MR->addr - (const char*)0;
...
public:
    ...
    void PostReceive(TPtrArg<TMemoryRegion> mem, ui64 id, const void *buf, size_t len)
    {
        ...
        ibv_recv_wr wr, *bad;
        ibv_sge sg;
        sg.addr = (const char*)buf - (const char*)0;
        sg.length = len;
        sg.lkey = mem->GetLKey();
        Zero(wr);
        wr.wr_id = id;
...
void MakeAH(ibv_ah_attr *res, TPtrArg<TIBPort> port, const TIPAddress &remoteAddr, const TIPAddress &localAddr, int serviceLevel);
...
public:
    ...
    TAddressHandle(TPtrArg<TIBPort> port, const TIPAddress &remoteAddr, const TIPAddress &localAddr, int serviceLevel)
        : IBCtx(port->GetCtx())
    {
        ibv_ah_attr attr;
    ...
    bool IsValid() const { return AH != 0; }
};
...
protected:
    ...
    {
        ui64 localAddr = (const char*)data - (const char*)0;
        ui32 lKey = 0;
        if (mem) {
...
public:
    ...
    void Init(const ibv_ah_attr &peerAddr, int peerQPN, int peerPSN)
    {
        ...
        attr.path_mtu = IBV_MTU_4096; // higher bandwidth
        // for connected QP
...
TVector<TIntrusivePtr<TIBPort>> GetIBDevices();
...
public:
    ...
    int GetBusId() const { return 0; }
    TPtrArg<TIBContext> GetCtx() { return null_ptr_arg; }
...
{
public:
    TMemoryRegion(TPtrArg<TIBContext>, void *, size_t) {}
    TMemoryRegion(TPtrArg<TIBContext>, ui64, int, size_t) {}
...
public:
    ...
    TAddressHandle(TPtrArg<TIBPort>, const TIPAddress &, const TIPAddress &, int) {}
    //ibv_ah *GetAH() { return AH; }
...
inline TVector<TIntrusivePtr<TIBPort>> GetIBDevices()
{
    return TVector<TIntrusivePtr<TIBPort>>();
}
#endif
}
