...
class TIBBufferPool : public TThrRefBase, TNonCopyable
{
    ...
    struct TSingleBlock
    {
        ...
        void Alloc(TPtrArg<TIBContext> ctx)
        {
            ...
            MemBuf.resize(dataSize);
            Mem = new TMemoryRegion(ctx, MemBuf.data(), dataSize);
...
public:
    TIBBufferPool(TPtrArg<TIBContext> ctx, int maxSRQWorkRequests) : IBCtx(ctx), FirstFreeBlock(0), PostRecvDeficit(maxSRQWorkRequests)
    {
    ...
    TPtrArg<TSharedReceiveQueue> GetSRQ() const { return SRQ; }
    ...
    void FreeBuf(ui64 idArg)
    {
        ...
        if (idArg & BP_AH_USED_FLAG) {
            Blocks[id >> BLOCK_SIZE_LN].AHHolder[id & (BLOCK_SIZE - 1)] = 0;
        }
    }
    ...
    int PostSend(TPtrArg<TRCQueuePair> qp, const void *data, size_t len)
    {
        ...
        if (len <= MAX_INLINE_DATA_SIZE) {
            qp->PostSend(null_ptr_arg, 0, data, len);
            return 0;
        } else {
            int id = AllocBuf();
    ...
    void PostSend(TPtrArg<TUDQueuePair> qp, TPtrArg<TAddressHandle> ah, int remoteQPN, int remoteQKey, const void *data, size_t len)
    {
        ...
        id |= BP_AH_USED_FLAG;
        if (len <= MAX_INLINE_DATA_SIZE) {
            qp->PostSend(ah, remoteQPN, remoteQKey, null_ptr_arg, id, data, len);
        } else {
            char *buf = blk.GetBufData(id);
            memcpy(buf, data, len);
    ...
    void RequestPostRecv() { ++PostRecvDeficit; }
...
public:
    TIBRecvPacketProcess(TPtrArg<TIBBufferPool> bp, const ibv_wc &wc) : BP(*bp.Get()), Id(wc.wr_id)
    {
    ...
    TIBRecvPacketProcess(TPtrArg<TIBBufferPool> bp, const ui64 wr_id) : BP(*bp.Get()), Id(wr_id) { Data = BP.GetBufData(Id); }
