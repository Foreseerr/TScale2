...
static TAtomic IBPortMutex;
static TVector<TIntrusivePtr<TIBPort>> IBPortArr;
static bool IBWasInitialized;

static int ReadBusId(const char *ibdev_path)
{
    TSeqReader f(Sprintf("%s/device/uevent", ibdev_path));
    if (!f.IsValid()) {
        return -1;
    }
    while (!f.IsEof()) {
        TString sz = f.ReadLine();
        if (StartsWith(sz, "PCI_SLOT_NAME")) {
            int domain = 0;
            int busId = 0;
            sscanf(sz.c_str(), "PCI_SLOT_NAME=%x:%x:", &domain, &busId);
            return busId;
        }
    }
    return -1;
...
TVector<TIntrusivePtr<TIBPort>> GetIBDevices()
{
    ...
    //    printf("\n");
    //}
    IBPortArr.resize(numDevices);
    for (yint devId = 0; devId < numDevices; ++devId) {
        int busId = ReadBusId(deviceList[devId]->ibdev_path);
        DebugPrintf("device %s, busId %d\n", deviceList[devId]->dev_name, busId);

        TIntrusivePtr<TIBContext> ctx;
        ctx = new TIBContext(deviceList[devId]);
        ...
        for (int port = 1; port <= devAttrs.phys_port_cnt; ++port) {
            ...
            if (portAttrs.state == IBV_PORT_ACTIVE) {
                IBPortArr[devId] = new TIBPort(ibContext, port, busId);
                break;
            }
        }
    }
